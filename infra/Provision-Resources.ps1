...
try {
    ...
    $result = az deployment group create --name "Deploy-$(Get-Random)" --resource-group $ResourceGroup --template-file $PSScriptRoot/main.bicep --parameters dataOwnerPrincipalId=$SignedInUserId dataReaderPrincipalId=$DataReaderPrincipalId | ConvertFrom-Json
